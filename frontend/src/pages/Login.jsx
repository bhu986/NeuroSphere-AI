...
import { Sparkles, Mail, Lock, Eye, EyeOff, Loader2, ArrowLeft, AlertCircle } from "lucide-react";
...
export function Login() {
  ...
  // Read URL search parameters on client side safely
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans selection:bg-blue-500 selection:text-white">
      ...
      >
        <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl -z-10 pointer-events-none" />
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-white/50 mt-1 font-medium text-center">Enter your details to access the cognitive dashboard</p>
        </div>
        ...
        {successMessage && (
          <motion.div 
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-start gap-3 text-xs text-emerald-400 font-extrabold shadow-sm"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center px-1">
              <label className="text-[11px] font-extrabold text-white/50 uppercase tracking-wider">Password</label>
              <button 
                type="button"
                className="text-[10px] font-extrabold text-blue-400 hover:text-blue-300 transition-colors"
                onClick={() => alert("Password reset is not configured for this demo environment.")}
              >
                Forgot Password?
              </button>
            </div>
